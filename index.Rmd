...
miles_davis$album_release_date <- substring(miles_davis$album_release_date, 1, 4)
...
#40ies
miles40_filtered <- select_if(miles40ies, is.numeric)
miles_composite40 <- miles40_filtered %>%
  scale() %>%
  as.data.frame() %>%
  mutate(compositescore = (danceability + acousticness + valence + energy + tempo) / 5)

which(miles_composite40[,18] < 0.01 & miles_composite40[,18] > -0.01)



#50ies
miles50_filtered <- select_if(Miles_early_50s, is.numeric)
miles_composite50 <- miles50_filtered %>%
  scale() %>%
  as.data.frame() %>%
  mutate(compositescore = (danceability + acousticness + valence + energy + tempo) / 5)

which(miles_composite50[,18] < 0.01 & miles_composite50[,18] > -0.01)
...
which(miles_compositemodal[,17] < 0.005 & miles_compositemodal[,17] > -0.005)




#herbie
milesherbie_filter <- select_if(miles_herbie, is.numeric)
milesherbie_composite <- milesherbie_filter %>%
  scale() %>%
  as.data.frame() %>%
  mutate(compositescore = (danceability + acousticness + valence + energy + tempo) / 5)

which(milesherbie_composite[,17] < 0.005 & milesherbie_composite[,17] > -0.005)



#Miles fusion
milesfusion_filter <- select_if(miles_fusion, is.numeric)
milesfusion_composite <- milesfusion_filter %>%
  scale() %>%
  as.data.frame() %>%
  mutate(compositescore = (danceability + acousticness + valence + energy + tempo) / 5)

which(milesfusion_composite[,17] < 0.005 & milesfusion_composite[,17] > -0.005)


#Miles Death
milesdeath_filter <- select_if(miles_death_clean, is.numeric)
milesdeath_composite <- milesdeath_filter %>%
  scale() %>%
  as.data.frame() %>%
  mutate(compositescore = (danceability + acousticness + valence + energy + tempo) / 5)

which(milesdeath_composite[,17] < 0.01 & milesdeath_composite[,17] > -0.01)


#Cleaning up Miles death with reissues:
miles_death_clean <- miles_death
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "'58 Sessions: Featuring Stella")),]
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "First Miles (Reissue - Bonus Tracks)")),]
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "First Miles (Reissue)")),]
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "Bags' Groove")),]
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "Directions")),]
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "Stockholm 1960 Complete")),]
miles_death_clean <- miles_death_clean[-c(which(miles_death_clean$album_name == "Our Delight")),]
...
milesmodal <- subset(miles_davis, album_release_date > 1954 & album_release_date < 1963)
milesmodal_short <- as.data.frame(milesmodal[,c(9:19, 30)])
milesmodal_short$Time_Period <- "1955-1962"


#second quintet 1963-1968
miles_herbie <- subset(miles_davis, album_release_date > 1963 & album_release_date < 1969)
miles_herbie_short <- as.data.frame(miles_herbie[,c(9:19, 30)])
miles_herbie_short$Time_Period <- "1963-1968"

#Milesfusion:
miles_fusion <- subset(miles_davis, album_release_date > 1969 & album_release_date < 1975)
miles_fusionshort <- as.data.frame(miles_fusion[,c(9:19, 30)])
miles_fusionshort$Time_Period <- "1969-1975"


#80ies_todeath
miles_death <- subset(miles_davis, album_release_date > 1979 & album_release_date < 1993)
...
miles_long <- rbind(miles40ies_cut, milesE50ies, milesmodal_short, miles_herbie_short, miles_fusionshort, miles_death_short)
...
#6
Miles_slope[1,6] <- mean(miles_death_short[,1])
Miles_slope[2,6] <- mean(miles_death_short[,2])
Miles_slope[3,6] <- mean(miles_death_short[,6])
Miles_slope[4,6] <- mean(miles_death_short[,7])
Miles_slope[5,6] <- mean(miles_death_short[,8])
Miles_slope[6,6] <- mean(miles_death_short[,9])
Miles_slope[7,6] <- mean(miles_death_short[,10])
...
plot <- ggplot(miles_long, aes(x = danceability, y = energy, col = valence, label = track_name)) + 
