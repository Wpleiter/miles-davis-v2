...
### The Musically shapeshifting genius that is Miles Davis {data-commentary-width=1100}
![Self Portrait by Miles Davis](Miles 3.png)
...
tempo <- ggplot(miles_long, aes(x = Time_Period, y = tempo, fill = Time_Period), main = "Miles' Tempos") + 
 geom_boxplot(show.legend = FALSE) + 
   xlab("Time Period") + ylab("Tempo in bpm") + 
  ggtitle("Miles' Tempos") + 
  Spotify_SSM +
...
Modes <- ggplot(miles_long, aes(x = mode, fill = Time_Period), main = "Miles' Tempos") + 
...
keys <- ggplot(miles_long, aes(x = key, fill = key), main = "Miles' Tempos") + 
 geom_bar(show.legend = FALSE)  + ylab("Count") +
  ggtitle("Miles' Keys") + 
  Theme_facet +
  theme(axis.text.x = element_text(angle = 0)) +
...
### Zooming out: Summarizing low level phase features{data-commentary-width=500}
